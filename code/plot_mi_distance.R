...
typology = read.csv("../data/typology.csv") %>% select(lang, lang_name)
d = read.csv("../output/hdmi_distance.csv") %>%
  select(-X) %>%
...
ggsave("../output/hdmi_position_mi.pdf", height=6, width=9)
...
ggsave("../output/hdmi_position.pdf", height=6, width=9)
