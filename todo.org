...
*** TODO print stats data as csv
    1. [X] main calls
    2. [ ] subcalls

*** TODO macro for running a benchmark on increasingly larger data
...
*** TODO get some actual results

** TODO coroutines for data pipelining
   So I think the coroutines would just wrap the following sequence:
   1. pushing the data in
   2. get skipgram
   3. flat mapping them
   4. runnng the counter

   And it seems like this is a beneficial strategy because it
   a). doesn't constantly create and destroy functions on the stack
       the way they are in a nested function call pipeline
   b). allows you to work on multiple stages at once, e.g. pushing data in
       while you get skipgrams, while you flat map a bunch of grams, while
       you run the counter
